...
from sqlalchemy import and_, func, or_, text  # create_engine,
...
from db.dbefclasses import URLCatalog  # , Base  # ORM models
...
def catalog_cache(
    ...
    """
    try:
        logger.info(f"{i()}🔍🔍🖥️💾 Fetching and processing URL: {url}")
        logger.info(f"{i()}🔍🔍🖥️💾 HTML content type: {type(html_content)}")

        if html_content is None:
            logger.warning(f"{i()}💥🚩 html_content is None for URL: {url}")

        if isinstance(html_content, (BeautifulSoup, Tag)):
            html_content = str(html_content)
        elif not isinstance(html_content, str):
            logger.info(
                f"{i()}💥💥💥CATALOG HTML ----- Cataloging cache data for URL: {url}"
            )
            logger.info(
                f"{i()}💥💥💥CATALOG HTML ----- html_content: {type(html_content)}"
            )
        ...
        with gs.read() as readsession:
            ...
            if existing_record:
                logger.info(
                    f"{i()}🔧🗃️ UPDATING HTML ----- Updating cache data for URL: {url}"
                )
                existing_record.html_content = html_content
            else:
                logger.info(
                    f"{i()}🔧🗃️ CATALOG HTML ----- Cataloging cache data for URL: {url}"
                )
                ...
                with gs.write() as sub_session:
                    ...
                    sub_session.commit()
                    logger.info(
                        f"{i()}🔧🗃️ A new record: HTML data saved for URL: {url}"
                    )
    except Exception as e:
        with gs.write() as writesession:
            writesession.rollback()
        logger.error(f"{i()}💥🚩Error cataloging cache data: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")


def update_url_status(url: str, completed: bool = False) -> None:
    """
    ...
    try:
        logger.info(
            f"{i()}🔧🗃️ UPDATING URL STATUS ------ URL: {url}, Completed: {completed}"
        )
        with gs.read() as readsession:
            ...
            if record:
                ...
                with gs.write() as sub_session:
                    sub_session.commit()
                    logger.info(
                        f"{i()}🔧🗃️ UPDATING URL STATUS ------ URL status updated: {url}"
                    )
            else:
                logger.warning(f"{i()}💥🚩URL not found for update: {url}")
    except Exception as e:
        with gs.write() as writesession:
            writesession.rollback()
            logger.error(f"{i()}💥🚩Error updating URL status: {e}")
            logger.error(f"{i()}{traceback.format_exc()}")
...
def check_url_exists(url: str) -> bool:
    ...
    try:
        ...
        logger.error(f"{i()}💥🚩Error checking URL existence: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")
    return exists


def url_cached(url: str) -> bool:
    """
    ...
    try:
        with gs.read() as readsession:
            is_cached = (
                readsession.query(URLCatalog)
                .filter(URLCatalog.url == url, URLCatalog.html_content.isnot(None))
                .count()
                > 0
            )
    except Exception as e:
        logger.error(f"{i()}💥🚩Error checking URL cache status: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")
    logger.info(f"{i()}🔍 URL CACHED ----- URL: {url}, is_cached: {is_cached}")
    return is_cached


def load_cache(url: str) -> BeautifulSoup:
    """
    ...
    try:
        with gs.read() as readsession:
            ...
            if existing_record and existing_record.html_content:
                logger.info(
                    f"{i()}🔧🗃️ LOADING HTML ----- Loading cache data for URL: {url}"
                )
                return BeautifulSoup(existing_record.html_content, "html.parser")
            else:
                logger.warning(f"{i()}💥🚩Error: No cached HTML found for URL: {url}")
                return None
    except Exception as e:
        logger.error(f"{i()}💥🚩Error loading cache html: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")
        return None


def build_conditions(  # todo must merge the two versions of this function one is right the other.....
    where: list,
) -> List[Any]:
    """
...
def get_uncached_urls(
    ...
    try:
        with gs.read() as readsession:
            ...
            return [
                (record.url, record.company_number, record.jurisdiction)
                for record in records
            ]
    except Exception as e:
        logger.error(f"{i()}💥🚩 Error retrieving URLs with null html_content: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")
        return []


def get_incomplete_urls(limit: int = 1000) -> list:
    """
    ...
    try:
        with gs.read() as readsession:
            urls = (
                readsession.query(URLCatalog.url)
                .filter(URLCatalog.completed is False)
                .limit(limit)
                .all()
            )
            logger.info(
                f"{i()}🌎📡Retrieved {len(urls)} URLs with completed marked as False."
            )
            return [url[0] for url in urls]
    except Exception as e:
        logger.error(
            f"{i()}💥🚩Error retrieving URLs with completed marked as False: {e}"
        )
        logger.error(f"{i()}{traceback.format_exc()}")
        return []
...
#     If the URL already exists, skip it to avoid UNIQUE constraint errors.
...
#     """
#     try:
#         logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Saving {len(urls)} URLs.")
#         new_url_records = []
#         existing_urls = set()  # Cache existing URLs to avoid duplicate queries

#         with gs.read() as readsession:
#             # Fetch all existing URLs in one go, which are faster than querying one by one
#             existing_urls_query = readsession.query(URLCatalog.url).filter(URLCatalog.url.in_(urls)).all()
...
#                 writesession.commit()
#                 logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Successfully saved {len(new_url_records)} URLs.")
#         else:
#             logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ No new URLs to save.")


#     except Exception as e:
#         with gs.write() as writesession:
#             writesession.rollback()
#             logger.error(f"{i()}💥🚩Error bulk saving URLs: {e}")
#             logger.error(f"{i()}{traceback.format_exc()}")
def bulk_save_urls(
    urls: List[str], company_number: str, jurisdiction: str, parent_id: int = None
) -> None:
    """
    Save a list of URLs with their associated company number and jurisdiction to the URLCatalog.
    Uses 'INSERT OR IGNORE' to handle duplicate URLs.
    ...
    try:
        logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Saving {len(urls)} URLs.")
        logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Parent ID: {parent_id}")
        logger.info(
            f"{i()}🔧🗃️ BULK SAVING URLS ------ Company Number: {company_number}"
        )
        logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Jurisdiction: {jurisdiction}")

        # Prepare the SQL insert query with INSERT OR IGNORE
        query = """
        INSERT OR IGNORE INTO url_catalog (url, company_number, jurisdiction, completed, parent_id)
        VALUES (:url, :company_number, :jurisdiction, 0, :parent_id)
        """

        # Build the parameter set for each URL
        params = [
            {
                "url": url,
                "company_number": company_number,
        ...
        # Execute the raw SQL query using a write session
        with gs.write() as writesession:
            writesession.execute(text(query), params)
            writesession.commit()
            logger.info(f"{i()}🔧🗃️ BULK SAVING URLS ------ Successfully saved URLs.")
...
def get_base_url_id(company_number: str, jurisdiction: str = None) -> int:
    """
    ...
    try:
        ...
        logger.error(f"{i()}Error fetching base URL ID: {e}")
        logger.error(f"{i()}{traceback.format_exc()}")
        return None
