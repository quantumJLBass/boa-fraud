...
def clean_zip_code(part):
    # Detect ZIP+4 code and remove "-0000" if present
    zip_match = re.match(r"(\d{5})(-0000)?$", part)
    if zip_match:
        return zip_match.group(1)  # Return only the 5-digit ZIP code
    return part
...
def validate_city_zip(city: str, state: str, zip_code: str) -> bool:
    """
    ...
    zip_code = (
        zip_code.zfill(5).replace(",", "").strip()
    ...
    for record in city_zip_data:
        if (
            ...
            and str(record["zip_code"]).zfill(5) == zip_code
        ):
            if fuzz.ratio(record["city"].upper(), city.upper()) > 90:
                logger.debug(
                    f"{i()}✅🏢🟰🏢Validated city: `{city}`, state: `{state}`, zip: `{zip_code}` against record: {record}"
                )
                return True
    logger.warning(
        f"{i()}‼️🚨🏢🙅‍♂️🏣🚨‼️🤔 City name validation failed for: `{city}`, state: `{state}`, zip: `{zip_code}`"
    )
    return False
...
CORRECTNESS_PROMPT_TEMPLATE = load_file_content(
    "prompts/CORRECTNESS_PROMPT_TEMPLATE.txt"
)
...
def enforce_english_symbols(text: str) -> str:
    """
...
def apply_directionals_and_parts(parsed_address: dict) -> dict:
    """
    ...
    for label in ["Recipient", "PlaceName", "StreetName"]:
        if label in parsed_address:
            logger.debug(f"{i()}📬❓❓Address parts: {label}")
            parsed_address[label] = clean_and_join_parts(parsed_address, label)
            logger.debug(
                f"{i()}🌍📫〽️✒️📬 Cleaned and joined parts for label `{label}`: {parsed_address[label]}"
            )
    ...
    for key in ["StreetNamePreDirectional", "StreetNamePostDirectional"]:
        if key in parsed_address and parsed_address[key].upper() in directionals:
            logger.debug(
                f"{i()}📬❓❓Directional: {directionals[parsed_address[key].upper()]}"
            )
            parsed_address[key] = directionals[parsed_address[key].upper()]
            logger.debug(
                f"{i()}🌍📫✒️📬 Applied directional for `{key}`: {parsed_address[key]}"
            )
    ...
    for key in [
        ...
        if key in parsed_address and parsed_address[key].upper() in address_parts:
            logger.debug(
                f"{i()}📬❓❓Address parts: {address_parts[parsed_address[key].upper()]}"
            )
            parsed_address[key] = address_parts[parsed_address[key].upper()]
            logger.debug(
                f"{i()}🌍📫✒️📬 Applied address part for `{key}`: {parsed_address[key]}"
            )

    return parsed_address


def clean_and_join_parts(parsed_address: dict, label: str) -> str:
    """
    ...
    logger.debug(
        f"{i()}🌍📫〽️✒️📬🧼🫧🧽 parts ready to be cleaned and or joinned: {parts}"
    )
    if len(parts) > 1:
        joined_value = " ".join(parts)
        logger.debug(
            f"{i()}🌍📫〽️✒️📬🧼🫧🧽 Joined multiple parts for `{label}`: {joined_value}"
        )
        return joined_value
    elif len(parts) == 1:
...
def extract_zip_from_address_component(parsed_address: dict, component: str) -> dict:
    """
    ...
    if zip_match:
        ...
        logger.debug(
            f"{i()}🌍📫〽️✒️📬 Extracted zip code `{zip_code}` from `{component}`"
        )
    return parsed_address


def construct_reformatted_address(parsed_address) -> str:
    """
    ...
    full_address = (
        f"{box}, {street_address}, {occupancy}, {city}, {state}, {zip_code}".strip(
            ", "
        ).upper()
    )
...
def correct_city(city: str) -> str:
    ...
    if not re.match("^[A-Z ]+$", city):
        logger.warning(f"{i()}📬‼️🚨 Invalid characters in city name: `{city}`")
...
def correct_state(state: str) -> str:
    ...
    if not re.match("^[A-Z]{2}$", state):
        logger.warning(f"{i()}📬‼️🚨 Invalid characters in state name: `{state}`")
        state = re.sub(r"[^A-Z]", "", state).strip()[
            :2
        ]  # Attempt to clean the state name
...
def validate_and_correct_city_zip(
    ...
    """
    zip_code = (
        zip_code.split("-")[0].zfill(5).replace(",", "").strip()
    )  # Use the base ZIP code for matching
    city = city.replace(",", "").strip()  # Remove trailing commas and extra spaces
    state = state.replace(",", "").strip()  # Remove trailing commas and extra spaces
    city_zip_data = get_city_zip_data()
        ...
        if record_zip == zip_code:
            if record_state == state.upper():
                if fuzz.ratio(record_city, city.upper()) > 90:
                    ...
                    logger.debug(
                        f"{i()}🔄🏢🟰🏢Corrected city: `{record_city}` for zip: `{zip_code}` and state: `{state}`"
                    )
                    return record_city, state, True
            elif record_city == city.upper():
                logger.debug(
                    f"{i()}🔄🏢🟰🏢Corrected state: `{record_state}` for city: `{city}` and zip: `{zip_code}`"
                )
                return city, record_state, True

    logger.warning(
        f"{i()}‼️🚨🏢🙅‍♂️🏣🚨‼️🤔 City/state validation failed for: `{city}`, state: `{state}`, zip: `{zip_code}`"
    )
    return city, state, False


def validate_and_retry(reformatted, original_address, retries=3) -> str:
    """
    ...
    for _ in range(retries):
        ...
        logger.debug(f"{i()}🌍📫〽️✒️📬reformatted: {reformatted}")
        ...
        logger.debug(f"{i()}🌍📫〽️✒️📬Validated Address: `{validated_address}`")
        ...
        logger.debug(f"{i()}🌍📫〽️✒️📬validated_address: `{validated_address}`")
        ...
        logger.debug(f"{i()}🌍📫〽️✒️📬Correctness Data: `{correctness_data}`")
        ...
        corrected_lines = [
            line for line in correctness_data if "Corrected Address" in line
        ]
        if corrected_lines:
            corrected_address = "\n".join(
                line.split(": ")[1] for line in corrected_lines if ": " in line
            ).strip()
            logger.debug(f"{i()}🌍📫〽️✒️📬Corrected Address: `{corrected_address}`")
            reformatted = corrected_address
        else:
            logger.warning(
                f"{i()}‼️🛑🚨🚩‼️🤔Corrected Address not found in Correctness Data: `{correctness_data}`"
            )
            break
...
def normalize_address(address: str) -> str:
    ...
    try:
        ...
        logger.debug(f"{i()}🌍📫〽️✒️📬 Preped Address: `{address}`")
        ...
        try:
            ...
            parsed_address = extract_zip_from_address_component(
                parsed_address, "PlaceName"
            )
            parsed_address = extract_zip_from_address_component(
                parsed_address, "StateName"
            )
            logger.debug(f"{i()}📫〽️✒️📬Address Type: [green]{address_type}")
            logger.debug(f"{i()}📫〽️✒️📬Parsed Address: [green]{parsed_address}")
            ...
            if "StateName" not in parsed_address or not parsed_address["StateName"]:
                if "ZipCode" in parsed_address and parsed_address["ZipCode"]:
                    ...
                    state = next(
                        (
                            record["state"]
                            for record in get_city_zip_data()
                            if str(record["zip_code"]).zfill(5) == zip_code.zfill(5)
                        ),
                        "",
                    )
                    parsed_address["StateName"] = state.upper() if state else ""
                    logger.debug(
                        f"{i()}🌍📫〽️✒️📬Inferred StateName: `{parsed_address['StateName']}` from ZipCode: `{zip_code}`"
                    )

        except usaddress.RepeatedLabelError as e:
            logger.error(
                f"{i()}🚨🚩🌍📫〽✒️📬️🛑Error in normalize_address (USAddress): {e}"
            )
        ...
        if "Recipient" in parsed_address:
            try:
                ...
                logger.debug(f"{i()}🔹🌍📫〽️✒️📬Parsed Person Data: {parsed_pp}")
                logger.debug(f"{i()}🔹🌍📫〽️✒️📬Person Type: {pp_type}")
                ...
                logger.error(
                    f"{i()}🔹🚨🚩🌍📫〽✒️📬️🛑Original String: {e.original_string}"
                )
                logger.error(f"{i()}🔹🚨🚩🌍📫〽✒️📬️🛑Parsed String: {e.parsed_string}")
                logger.error(traceback.format_exc())
        ...
        logger.debug(
            f"{i()}✒️📬 Parsed Address (after cleaning and joining): {parsed_address}"
        )
        ...
        if not valid:
            logger.warning(
                f"{i()}✒️📬🛑❌🚨🚩 Invalid city/state or zip code detected: `{city}`, `{state}`, `{zip_code}`"
            )
        ...
        logger.debug(
            f"{i()}🌍📫〽️✒️📬 Corrected Address: City: `{city}`, State: `{state}`, Zip: `{zip_code}`"
        )
        ...
        validated_address = (
            validated_address.replace(",,", ",").replace(", ,", ",").replace(", ,", ",")
        )
        validated_address = re.sub(
            r"\s{2,}", " ", validated_address
        )  # Replace multiple spaces with a single space
        logger.debug(
            f"{i()}✒️📬🌍📫〽️✒️📬FINAL REFORMATTED ADDRESS: `{validated_address}`"
        )
        ...
        logger.error(f"{i()}✒️📬🛑🚨✒️📬️🤔Exception in normalize_address: {e}")
        logger.error(traceback.format_exc())
        return address  # Return the original address if an error occurs
