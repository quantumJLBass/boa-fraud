...
class Jurisdiction(Base, CatalogMixin):
    ...
    )  # Unique jurisdiction code ex: us_id , 83501
    ...
    )  # Area code of telephone number in the jurisdiction (if applicable)
    parent_jurisdiction = Column(
        Integer, ForeignKey("jurisdiction.id", ondelete="SET NULL"), nullable=True
    )  # Code of the parent jurisdiction
    ...
    related_relationships = relationship(
        "RelatedRelationship", back_populates="jurisdiction"
    )
    subsidiary_relationships = relationship(
        "SubsidiaryRelationship", back_populates="jurisdiction"
    )
    ...
    control_statements_controlled = relationship(
        "ControlStatement",
        foreign_keys="ControlStatement.controlled_company_jurisdiction",
        back_populates="controlled_jurisdiction",
    )
    control_statements_controlling = relationship(
        "ControlStatement",
        foreign_keys="ControlStatement.controlling_company_jurisdiction",
        back_populates="controlling_jurisdiction",
    )
...
class Company(Base, CatalogMixin):
    ...
    parent_company_name = Column(
        String, nullable=True, default=None
    )  # Name of the parent company
    parent_company_url = Column(
        String, nullable=True, default=None
    )  # URL of the parent company

    # Add constraints
    __table_args__ = (
        CheckConstraint(
            "incorporation_date GLOB '[0-9][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]' OR incorporation_date IS NULL",
            name="incorporation_date_format",
        ),
    )
    ...
    trademark_registrations = relationship(
    ...
    jurisdiction = relationship(
        "Jurisdiction", back_populates="companies", cascade="none", passive_deletes=True
    )
    officers = relationship(
        "Officer",
        back_populates="company",
        foreign_keys="Officer.company_number",
        cascade="none",
        passive_deletes=True,
    )
    events = relationship(
        "Event",
        back_populates="company",
        foreign_keys="Event.company_number",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    share_parcels = relationship(
        "ShareParcel",
        back_populates="owning_company",
        foreign_keys="ShareParcel.company_number",
        cascade="none",
        passive_deletes=True,
    )
    share_owners = relationship(
        "ShareParcel",
        back_populates="company_shareholder",
        foreign_keys="ShareParcel.shareholder_id_company",
        cascade="none",
        passive_deletes=True,
    )
    related_relationships = relationship(
        "RelatedRelationship",
        foreign_keys="RelatedRelationship.company_number",
        back_populates="company",
        cascade="none",
        passive_deletes=True,
    )
    subsidiaries = relationship(
        ...
        back_populates="company",
        foreign_keys="SubsidiaryRelationship.company_number",
        cascade="none",
        passive_deletes=True,
    )
    addresses = relationship(
        "CompanyAddress",
        back_populates="company",
        foreign_keys="CompanyAddress.company_number",
        cascade="none",
        passive_deletes=True,
    )
    people = relationship(
        "CompanyPerson",
        back_populates="company",
        foreign_keys="CompanyPerson.company_number",
        cascade="none",
        passive_deletes=True,
    )
    officers_assoc = relationship(
        "CompanyOfficer",
        back_populates="company",
        foreign_keys="CompanyOfficer.company_number",
        cascade="none",
        passive_deletes=True,
    )
    url_catalog = relationship(
        "URLCatalog",
        back_populates="company",
        foreign_keys="URLCatalog.company_number",
        cascade="none",
        passive_deletes=True,
    )
    control_statements = relationship(
        "ControlStatement",
        back_populates="company",
        foreign_keys="ControlStatement.company_number",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    mergers = relationship(
        "Merger",
        back_populates="company",
        foreign_keys="Merger.company_number",
        cascade="none",
        passive_deletes=True,
    )
    mergers_as_merged = relationship(
        "Merger",
        foreign_keys="Merger.merged_company_id",
        back_populates="merged_company",
        cascade="none",
        passive_deletes=True,
    )
    mergers_as_surviving = relationship(
        "Merger",
        foreign_keys="Merger.surviving_company_id",
        back_populates="surviving_company",
        cascade="none",
        passive_deletes=True,
    )
    gazette_notices = relationship(
        "GazetteNotice",
        back_populates="company",
        foreign_keys="GazetteNotice.company_number",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    total_shares = relationship(
        "TotalShare",
        back_populates="company",
        foreign_keys="TotalShare.company_number",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    publications = relationship(
        "Publication",
        back_populates="company",
        foreign_keys="Publication.company_number",
        cascade="none",
        passive_deletes=True,
    )
    identifier_delegate = relationship(
        "IdentifierDelegate",
        back_populates="company",
        foreign_keys="IdentifierDelegate.company_number",
        cascade="none",
        passive_deletes=True,
    )
    akas = relationship(
...
class Address(Base, CatalogMixin):
    ...
    officer_addresses = relationship(
        "OfficerAddress",
        foreign_keys="OfficerAddress.address_id",
        back_populates="address",
        cascade="all, delete-orphan",
    )
    company_addresses = relationship(
        "CompanyAddress",
        foreign_keys="CompanyAddress.address_id",
        back_populates="address",
        cascade="all, delete-orphan",
    )
    people_addresses = relationship(
        "PeopleAddress",
        foreign_keys="PeopleAddress.address_id",
        back_populates="address",
        cascade="all, delete-orphan",
    )
...
class Officer(Base, CatalogMixin):
    ...
    __hidden_columns__ = ["link", "address"]  # Columns to be hidd
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the officer
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    ...
    __table_args__ = (
        CheckConstraint(
            "start_date GLOB '[0-9][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]' OR start_date IS NULL",
            name="start_date_format",
        ),
        CheckConstraint(
            "end_date GLOB '[0-9][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]' OR end_date IS NULL",
    ...
    addresses = relationship(
        "OfficerAddress", back_populates="officer", cascade="all, delete-orphan"
    )
    person_officers = relationship(
        "PersonOfficer", back_populates="officer", cascade="all, delete-orphan"
    )
    company_officers = relationship(
        "CompanyOfficer", back_populates="officer", cascade="all, delete-orphan"
    )
...
class Person(Base, CatalogMixin):
    ...
    # Relationships
    share_parcels = relationship(
        "ShareParcel", back_populates="person", cascade="all, delete-orphan"
    )
    addresses = relationship(
        "PeopleAddress", back_populates="person", cascade="all, delete-orphan"
    )
    officers = relationship(
        "PersonOfficer", back_populates="person", cascade="all, delete-orphan"
    )
    akas = relationship(
        "PersonAKA", back_populates="person", cascade="all, delete-orphan"
    )  # Added relationship
    company_people = relationship(
        "CompanyPerson", back_populates="person", cascade="all, delete-orphan"
    )
...
class PersonAKA(Base, CatalogMixin):
    ...
    __hidden_columns__ = ["type", "language"]  # Columns to be hidden
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the alternate name record
    person_id = Column(
        Integer, ForeignKey("people.id"), nullable=False
    )  # Identifier for the person
    aka = Column(String, nullable=False)  # Alternate name (Also Known As)
    start_date = Column(
        Date, default=None, nullable=True
    )  # Start date of the alternative name
    end_date = Column(
        Date, default=None, nullable=True
    )  # End date of the alternative name
    type = Column(String(12), nullable=True)  # Type of the alternative name
    language = Column(
        String(2), default="en", nullable=True
    )  # Language code, 2 characters long
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "type IN ('trading', 'abbreviation', 'legal', 'published', 'dba', 'aka', 'alias') OR type IS NULL",
            name="person_aka_type",
        ),
    )
...
class CompanyAKA(Base, CatalogMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "type IN ('trading', 'abbreviation', 'legal', 'published', 'dba', 'aka', 'alias') OR type IS NULL",
            name="company_aka_type",
        ),
    )

    # Relationships
    company = relationship(
        "Company", back_populates="akas"
    )  # Assuming there's a back_populates in Company
...
class Attribute(Base, CatalogMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "company_number", "name", "value", name="uix_company_name_value"
        ),
    )
...
class Event(Base, CatalogMixin):
    ...
    __hidden_columns__ = ["link"]  # Columns to be hidd
...
class IdentifierDelegate(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the identifier delegate
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    )  # Unique identifier for the company
    identifier_system = Column(
        String, default=None, nullable=True
    )  # System of the identifier
    identifier = Column(String, default=None, nullable=True)  # Identifier value
    categories = Column(
        String, default=None, nullable=True
    )  # Categories of the identifier
    statement_link = Column(String, nullable=True, default=None)  # Statement link URL
    external_link = Column(String, nullable=True, default=None)  # External link URL
...
class Assertion(Base, CatalogMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "title",
            "description",
            name="uix_assertions_company_title_description",
        ),
    )
...
class Filing(Base, CatalogMixin):
    ...
    __hidden_columns__ = ["link"]  # Columns to be hidden
    id = Column(
        Integer, primary_key=True, autoincrement=True
...
class TrademarkRegistration(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the trademark registration
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    )  # Unique identifier for the company
    ...
    company = relationship("Company", back_populates="trademark_registrations")
...
class IndustryCode(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the industry code
    ...
    companies = relationship("CompanyIndustryCode", back_populates="industry_code")
...
class CompanyIndustryCode(Base, CatalogMixin):
    ...
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="CASCADE", onupdate="NO ACTION"
        ),
        nullable=False,
    )
    industry_code_id = Column(
        Integer,
        ForeignKey("industry_codes.id", ondelete="CASCADE", onupdate="NO ACTION"),
        nullable=False,
    )  # Unique identifier for the industry code
    statement_link = Column(String, nullable=True)  # Link to the statement

    __table_args__ = (PrimaryKeyConstraint("company_number", "industry_code_id"),)
...
class Classification(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique primarykey identifier for the classification
...
class Publication(Base, CatalogMixin):
    ...
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    )  # Company number of the publication

    # Relationships
    company = relationship("Company", back_populates="publications")
    gazette_notices = relationship(
        "GazetteNotice", back_populates="publication", cascade="all, delete-orphan"
    )
...
class GazetteNotice(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the gazette notice
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    )  # Unique identifier for the company
    ...
    publication_id = Column(
        Integer, ForeignKey("publications.id")
    )  # Unique identifier for the publication
    notice = Column(Text)  # Notice text
    classification_id = Column(
        Integer, ForeignKey("classifications.id")
...
class TotalShare(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the total shares
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=False
    )  # Unique identifier for the company
...
class RelatedRelationship(Base, CatalogMixin):
    ...
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=False,
    ...
    jurisdiction_code = Column(
        String,
        ForeignKey("jurisdiction.code", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    )  # Jurisdiction code
    ...
    # Relationships
    company = relationship(
        "Company", foreign_keys=[company_number], back_populates="related_relationships"
    )
...
class ShareParcel(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the share parcel
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
    ...
    shareholder_id_person = Column(
        Integer,
        ForeignKey("people.id", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    )  # Unique identifier for the shareholder (person)
    shareholder_id_company = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=True,
    )  # Unique identifier for the shareholder (company)
    ...
    # Relationships
    person = relationship(
        "Person", back_populates="share_parcels", foreign_keys=[shareholder_id_person]
    )  # Relationship to Person
    company_shareholder = relationship(
        "Company", back_populates="share_owners", foreign_keys=[shareholder_id_company]
    )  # Relationship to Company as Shareholder
    owning_company = relationship(
        "Company", back_populates="share_parcels", foreign_keys=[company_number]
...
class SubsidiaryRelationship(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the subsidiary relationship
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
    ...
    subsidiary_company_id = Column(
        ...
        nullable=True,
    )  # Unique identifier for the subsidiary company
    jurisdiction_code = Column(
        String,
        ForeignKey("jurisdiction.code", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    )  # Jurisdiction code for the subsidiary company
    ...
    link = Column(
        String, nullable=True
    )  # Link to more information about the subsidiary company
    start_date = Column(
        String, default=None, nullable=True
    )  # Start date of the subsidiary relationship
    end_date = Column(
        String, default=None, nullable=True
    )  # End date of the subsidiary relationship
    statement_link = Column(String, nullable=True)  # Statement link
    ...
    __table_args__ = (
        CheckConstraint(
            "start_date GLOB '[0-9][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]' OR start_date IS NULL",
            name="start_date_format",
        ),  # Format constraint for start date
        CheckConstraint(
            "end_date GLOB '[0-9][0-9][0-9][0-9]-[0-1][0-9]-[0-3][0-9]' OR end_date IS NULL",
            name="end_date_format",
        ),  # Format constraint for end date
    )

    # Relationships
    company = relationship(
        "Company", foreign_keys=[company_number], back_populates="subsidiaries"
    )  # Relationship to Owning Company
    subsidiary_company_obj = relationship(
        "Company", foreign_keys=[subsidiary_company_id]
    )  # Relationship to Subsidiary Company
    jurisdiction = relationship(
        "Jurisdiction", back_populates="subsidiary_relationships"
...
class Merger(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the merger
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
    ...
    merged_company_id = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=True,
    )  # Unique identifier for the merged company
    surviving_company_name = Column(
        String, nullable=True
    )  # Name of the surviving company
    surviving_company_id = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=True,
    )  # Unique identifier for the surviving company
    jurisdiction_id = Column(
        Integer,
        ForeignKey("jurisdiction.id", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    ...
    # Relationships
    company = relationship(
        "Company", foreign_keys=[company_number], back_populates="mergers"
    )  # Relationship to Owning Company
    merged_company = relationship(
        "Company", foreign_keys=[merged_company_id], back_populates="mergers_as_merged"
    )  # Relationship to Merged Company
    surviving_company = relationship(
        "Company",
        foreign_keys=[surviving_company_id],
        back_populates="mergers_as_surviving",
    )  # Relationship to Surviving Company
    jurisdiction = relationship(
        "Jurisdiction", foreign_keys=[jurisdiction_id], back_populates="mergers"
    )  # Relationship to Jurisdiction
...
class ControlStatement(Base, CatalogMixin):
    ...
    __hidden_columns__ = [
        ...
        "controlled_company_jurisdiction",
        "controlling_company_number",
        "controlling_company_jurisdiction",
        "mechanisms",
        "statement_link",
    ]  # Columns to be hidden
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the control statement
    company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
    ...
    controlled_company_name = Column(
        String, nullable=True
    )  # Name of the controlled company
    controlled_company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=False,
    )  # Unique identifier for the controlled company
    controlled_company_jurisdiction = Column(
        String,
        ForeignKey("jurisdiction.code", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    )  # Jurisdiction code for the controlled company
    controlling_company_name = Column(
        String, nullable=True
    )  # Name of the controlling company
    controlling_company_number = Column(
        String,
        ForeignKey(
            "companies.company_number", ondelete="NO ACTION", onupdate="NO ACTION"
        ),
        nullable=False,
    )  # Unique identifier for the controlling company
    controlling_company_jurisdiction = Column(
        String,
        ForeignKey("jurisdiction.code", ondelete="NO ACTION", onupdate="NO ACTION"),
        nullable=True,
    )  # Jurisdiction code for the controlling company
    start_date = Column(
        String, default=None, nullable=True
    )  # Start date of the control statement
    end_date = Column(
        String, default=None, nullable=True
    )  # End date of the control statement
    mechanisms = Column(Text, nullable=True)  # Control mechanisms used
    statement_link = Column(
        String, nullable=True
    )  # Link to the control statement document
    ...
    # Relationships
    company = relationship(
        "Company", foreign_keys=[company_number], back_populates="control_statements"
    )  # Relationship to Owning Company
    controlled_company = relationship(
        "Company", foreign_keys=[controlled_company_number]
    )  # Relationship to Controlled Company
    controlling_company = relationship(
        "Company", foreign_keys=[controlling_company_number]
    )  # Relationship to Controlling Company
    controlled_jurisdiction = relationship(
        "Jurisdiction",
        foreign_keys=[controlled_company_jurisdiction],
        back_populates="control_statements_controlled",
    )  # Relationship to Controlled Jurisdiction
    controlling_jurisdiction = relationship(
        "Jurisdiction",
        foreign_keys=[controlling_company_jurisdiction],
        back_populates="control_statements_controlling",
    )  # Relationship to Controlling Jurisdiction
...
class ControlMechanism(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the control mechanism
...
class ControlStatementMechanism(Base, CatalogMixin):
    ...
    control_statement_id = Column(
        Integer, ForeignKey("control_statements.id")
    )  # Identifier for the control statement
    mechanism_id = Column(
        Integer, ForeignKey("control_mechanisms.id")
    )  # Identifier for the control mechanism

    # Add primarykey constraint
    __table_args__ = (PrimaryKeyConstraint("control_statement_id", "mechanism_id"),)
...
class CompanyAddress(Base, CatalogMixin):
    ...
    address_id = Column(
        Integer,
        ForeignKey("addresses.id", ondelete="CASCADE", onupdate="NO ACTION"),
        primary_key=True,
    )  # Unique identifier for the address
    type = Column(
        String,
        ForeignKey("address_types.code", ondelete="SET NULL", onupdate="NO ACTION"),
        nullable=True,
    )  # Address type code
    ...
    # Relationships
    company = relationship(
        "Company", back_populates="addresses"
    )  # Relationship to the company
    address = relationship(
        "Address", back_populates="company_addresses"
    )  # Relationship to the address
    address_type = relationship(
        "AddressType", back_populates="company_addresses"
    )  # Relationship to the address type
...
class PeopleAddress(Base, CatalogMixin):
    ...
    address_id = Column(
        Integer, ForeignKey("addresses.id", ondelete="CASCADE"), primary_key=True
    )  # Identifier for the address
    type = Column(
        String, ForeignKey("address_types.code", ondelete="SET NULL"), nullable=True
    )  # Address type code
    start_date = Column(Date, default=None, nullable=True)  # Start date of the address
    end_date = Column(Date, default=None, nullable=True)  # End date of the address
    ...
    address = relationship(
        "Address", back_populates="people_addresses"
    )  # Relationship to the address
    address_type = relationship(
        "AddressType", back_populates="people_addresses"
    )  # Relationship to the address type
...
class OfficerAddress(Base, CatalogMixin):
    ...
    officer_id = Column(
        Integer, ForeignKey("officers.id", ondelete="CASCADE"), primary_key=True
    )
    address_id = Column(
        Integer, ForeignKey("addresses.id", ondelete="CASCADE"), primary_key=True
    )
...
class CompanyPerson(Base, CatalogMixin):
    ...
    company_number = Column(
        String,
        ForeignKey("companies.company_number", ondelete="CASCADE"),
        primary_key=True,
    ...
    role = Column(
        String, ForeignKey("role_types.code", ondelete="SET NULL"), nullable=True
    )  # Role type code
    ...
    # Relationships
    company = relationship(
        "Company", back_populates="people"
    )  # Relationship to the company
    person = relationship(
        "Person", back_populates="company_people"
    )  # Relationship to the person
    role_type = relationship(
        "RoleType", back_populates="company_person_roles"
    )  # Relationship to the role type
...
class RoleType(Base, CatalogMixin):
    ...
    company_person_roles = relationship(
        "CompanyPerson", back_populates="role_type", cascade="all, delete-orphan"
    )
    person_officer_roles = relationship(
        "PersonOfficer", back_populates="role_type", cascade="all, delete-orphan"
    )
    company_officer_roles = relationship(
        "CompanyOfficer", back_populates="role_type", cascade="all, delete-orphan"
    )
...
class AddressType(Base, CatalogMixin):
    ...
    # Backpopulates relationships
    company_addresses = relationship(
        "CompanyAddress", back_populates="address_type", cascade="all, delete-orphan"
    )
    people_addresses = relationship(
        "PeopleAddress", back_populates="address_type", cascade="all, delete-orphan"
    )
    officer_addresses = relationship(
        "OfficerAddress", back_populates="address_type", cascade="all, delete-orphan"
    )
...
class LEIRecord(Base, CatalogMixin):
    ...
    jurisdiction_id = Column(
        Integer, ForeignKey("jurisdiction.id")
    )  # Identifier for the jurisdiction
...
class LEIEntity(Base, CatalogMixin):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # Unique identifier for the LEI entity
...
class LEIEntityDetail(Base, CatalogMixin):
    ...
    lei = Column(
        String, ForeignKey("lei_data.lei")
...
class LEIAddress(Base, CatalogMixin):
    ...
    address_type = Column(
        String
    )  # Type of the address (e.g., registered, headquarters)
...
class URLCatalog(Base, CatalogMixin):
    ...
    company_number = Column(
        String, ForeignKey("companies.company_number"), nullable=True
    )  # Unique identifier for the company
    ...
    )  # Flag indicating if the URL has been processed
    html_content = Column(Text)  # HTML content of the page
    parent_id = Column(
        Integer, ForeignKey("url_catalog.id"), nullable=True
    )  # Identifier for the parent URL
...
def set_cached_on(target, value, oldvalue, _):
    """
