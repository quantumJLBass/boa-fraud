...
def parse_address(
    ...
    city, state, zip_code = parts[-3], parts[-2], parts[-1].split('-')[0]

    # Everything before the last three parts is the street information
    street_address = parts[0]  # Always present
    secondary_address = parts[1] if len(parts) > 4 else ""  # Only if there are 5 parts
...
def lookup_zipcode(
    ...
    if secondary:
        formatted_address = f"{street}, {secondary}, {city}, {state}, {zip_code}-{zip_plus4}"
    else:
