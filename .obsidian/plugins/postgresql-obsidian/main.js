...
// src/types/adapter-name.ts
var AdapterName = /* @__PURE__ */ ((AdapterName2) => {
  AdapterName2["PostgreSQL"] = "PostgreSQL";
  return AdapterName2;
})(AdapterName || {});
...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Adapter name").setDesc("Select your database adapter here. If yours doesn't appear, please open an issue on GitHub.").addDropdown((dropdown) => {
      const options = Object.keys(AdapterName).reduce((acc, key) => ({
        ...acc,
        [key]: key
      }), {});
      dropdown.addOptions(options);
      dropdown.onChange(async (value) => {
        this.plugin.settings.adapterName = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("PostgreSQL connection URL").setDesc("This connection URL will be used to send your data.").addText((text) => text.setPlaceholder("").setValue(this.plugin.settings.connectionUrl).onChange(async (url) => {
...
var DEFAULT_SETTINGS = {
  connectionUrl: "",
  adapterName: "PostgreSQL" /* PostgreSQL */
};
...
var Builder = class extends NotTagged {
  ...
  build(before, parameters, types2, options) {
    ...
    return keyword.fn(this.first, this.rest, parameters, types2, options);
  }
};
...
var builders = Object.entries({
  ...
  update(first, rest, parameters, types2, options) {
    return (rest.length ? rest.flat() : Object.keys(first)).map((x) => escapeIdentifier(options.transform.column.to ? options.transform.column.to(x) : x) + "=" + handleValue(first[x], parameters, types2, options));
  },
  insert(first, rest, parameters, types2, options) {
    const columns = rest.length ? rest.flat() : Object.keys(Array.isArray(first) ? first[0] : first);
    return "(" + columns.map((x) => escapeIdentifier(options.transform.column.to ? options.transform.column.to(x) : x)).join(",") + ")values" + valuesBuilder(Array.isArray(first) ? first : [first], parameters, types2, columns, options);
  }
...
function Connection(options, queues = {}, { onopen = noop, onend = noop, onclose = noop } = {}) {
  ...
  async function createSocket() {
    let x;
    try {
      x = options.socket ? await Promise.resolve(options.socket(options)) : import_net.default.Socket();
    } catch (e) {
      error(e);
      return;
    }
  ...
  function connected() {
    try {
      ...
      lifeTimer.start();
      socket.on("data", data);
      keep_alive && socket.setKeepAlive(true, 1e3 * keep_alive);
  ...
  function DataRow(x) {
    ...
    for (let i = 0; i < query.statement.columns.length; i++) {
      ...
      index += 4;
      value = length2 === -1 ? null : query.isRaw === true ? x.slice(index, index += length2) : column.parser === void 0 ? x.toString("utf8", index, index += length2) : column.parser.array === true ? column.parser(x.toString("utf8", index + 1, index += length2)) : column.parser(x.toString("utf8", index, index += length2));
...
function Subscribe(postgres2, options) {
  ...
  async function init(sql2, slot2, publications) {
    ...
    function data(x2) {
      if (x2[0] === 119)
        parse(x2.slice(25), state2, sql2.options.parsers, handle);
...
function parse(x, state, parsers2, handle) {
  ...
  Object.entries({
    ...
    U: (x2) => {
      ...
      old && (i = tuples(x2, old, key ? relation.keys : relation.columns, i += 3));
      const row = {};
      tuples(x2, row, relation.columns, i + 3);
      handle(row, {
        command: "update",
        relation,
        key,
...
function parseOptions(a, b2) {
  ...
  return {
    ...
    ...Object.entries(defaults).reduce((acc, [k, d]) => (acc[k] = k in o ? o[k] : k in query ? query[k] === "disable" || query[k] === "false" ? false : query[k] : env["PG" + k.toUpperCase()] || d, acc), {}),
    connection: {
...
var PostgreSQLAdapter = class {
  ...
  async insertPage(url, path, data) {
    const sql = await this._connect(url);
    try {
      await sql`INSERT INTO obsidian.file
			${sql({
        path,
        dataview_data: sql.json(data)
      })}
			ON CONFLICT (path)
			DO
...
var adapters = {
  ["PostgreSQL" /* PostgreSQL */]: new PostgreSQLAdapter()
};
async function constructAdapter(name) {
  const adapter = adapters[name];
  return adapter;
}

// src/classes/postgresql-plugin.ts
var PostgreSQLPlugin = class extends import_obsidian2.Plugin {
  settings;
  _adapter;
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        const adapter = await constructAdapter(this.settings.adapterName);
        const dv = (0, import_obsidian_dataview.getAPI)();
        if (!dv) {
          new import_obsidian2.Notice("The Dataview API is not available");
          return;
        }
        ...
        const dataviewData = dv.page(filepath);
        if (!dataviewData) {
          new import_obsidian2.Notice("Failed to fetch the dataview data");
          return;
        }
        ...
        try {
          ...
          await adapter.insertPage(this.settings.connectionUrl, filepath, dataviewData);
          new import_obsidian2.Notice(`${this.settings.adapterName}: Inserted page`);
        } catch (err) {
          new import_obsidian2.Notice(`${this.settings.adapterName} error: ${err}`);
        }
      }
    });
  }
...
// src/main.ts
var main_default = PostgreSQLPlugin;
